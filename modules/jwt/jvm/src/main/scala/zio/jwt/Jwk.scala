...
/**
 * JSON Web Key (RFC 7517) ADT.
 * Instances may be constructed directly or via [[Jwk$ Jwk]].from factory methods.
 */
...
object Jwk:
  ...
  private def decodeBigInt(b64: Base64UrlString): Either[JwtError, BigInteger] =
    Try {
      import scala.language.unsafeNulls
      val bytes = base64UrlDecoder.decode(b64.unwrap)
      BigInteger(1, bytes)
  ...
  private def encodeBigInt(value: BigInteger): Base64UrlString =
    import scala.language.unsafeNulls
  ...
  /**
   * Encodes a positive BigInteger as base64url, padded to the given field size in bytes.
   * Used for EC coordinates which must be padded to the curve's field element size.
   */
  private def encodeBigIntPadded(value: BigInteger, fieldSize: Int): Base64UrlString =
    import scala.language.unsafeNulls
    val bytes = value.toByteArray
    val unsigned = if bytes.length > 1 && bytes(0) == 0.toByte then bytes.drop(1) else bytes
    val padded =
  ...
  def from(key: PrivateKey, publicKey: PublicKey, kid: Option[Kid]): Either[JwtError, Jwk] =
    ...
    (key, publicKey) match
      ...
      case _ => Left(JwtError.MalformedToken(
  ...
  private def ecToPublicKey(crv: EcCurve, xB64: Base64UrlString, yB64: Base64UrlString): Either[JwtError, PublicKey] =
    for
      x     <- decodeBigInt(xB64)
      y     <- decodeBigInt(yB64)
      point  = ECPoint(x, y)
  ...
  private def ecToPrivateKey(crv: EcCurve, dB64: Base64UrlString): Either[JwtError, PrivateKey] =
    for
      d   <- decodeBigInt(dB64)
      key <- Try {
        import scala.language.unsafeNulls
  ...
  private def rsaToPublicKey(nB64: Base64UrlString, eB64: Base64UrlString): Either[JwtError, PublicKey] =
    for
      n   <- decodeBigInt(nB64)
      _   <- Either.cond(n.bitLength() >= MinRsaKeyBits, (), JwtError.MalformedToken(
  ...
  private def symmetricToSecretKey(kB64: Base64UrlString, alg: Option[Algorithm]): Either[JwtError, SecretKey] =
    Try {
      import scala.language.unsafeNulls
      val bytes = base64UrlDecoder.decode(kB64.unwrap)
  ...
  private def curveFromSpec(ecPub: JcaEcPublicKey): Either[JwtError, EcCurve] =
    import scala.language.unsafeNulls
  ...
  private def fromEcPublicKey(ec: JcaEcPublicKey, kid: Option[Kid]): Either[JwtError, Jwk] =
    import scala.language.unsafeNulls
  ...
  private def fromEcPrivateKey(ec: JcaEcPrivateKey, ecPub: JcaEcPublicKey, kid: Option[Kid]): Either[JwtError, Jwk] =
    import scala.language.unsafeNulls
  ...
  private def fromRsaPublicKey(rsa: JcaRsaPublicKey, kid: Option[Kid]): Either[JwtError, Jwk] =
    import scala.language.unsafeNulls
    ...
    Either.cond(
      ...
      JwtError.MalformedToken(IllegalArgumentException(
        s"RSA key must be at least $MinRsaKeyBits bits, got ${rsa.getModulus.bitLength()}"
      ))
    )

  private def fromRsaPrivateKey(rsa: JcaRsaPrivateCrtKey, rsaPub: JcaRsaPublicKey, kid: Option[Kid]): Either[JwtError, Jwk] =
    import scala.language.unsafeNulls
    Either.cond(
      ...
      Jwk.RsaPrivateKey(
        ...
        use = None, keyOps = None, alg = None, kid = kid
      ),
      JwtError.MalformedToken(IllegalArgumentException(
